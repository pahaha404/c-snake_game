...
//gate 충돌
int collision_gate(){
  int nextX = snake[0].x + head_way[Head_Direction][0];
  int nextY = snake[0].y + head_way[Head_Direction][1];

  //스네이크가 gate[0]으로 진입
  int in, out;
  if(nextX == gate[0].x && nextY == gate[0].y){
    in = 0; out = 1;
  ...
  //스네이크가 gate[1]로 진입
  else if(nextX == gate[1].x && nextY == gate[1].y){
    in = 1; out = 0;
    if(gate.size() > 2) gate.pop_back();
    gate.push_back(snakepart(in, out));
    return 7;
...
void generate_gate(){
  ...
  do{
    x2 = rand()%30;
    y2 = rand()%30;
  } while(map[stage_num][y2][x2] != 1 || (x1 == x2 && y1 == y2));
