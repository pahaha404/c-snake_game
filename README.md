...
이 프로젝트는 C++와 `ncurses` 라이브러리를 사용하여 개발되었습니다. 터미널 환경에서 실행하기 위해 아래 단계에 따라 환경을 설정하고 빌드해주세요.

### 1. ncurses 라이브러리 설치 (Linux / Ubuntu / WSL)
게임 화면을 터미널에 출력하기 위해 `ncurses` 라이브러리가 필요합니다. 먼저 터미널에 아래 명령어를 입력해 설치해주세요.
...
### 2. 프로젝트 클론 (Git Clone)
GitHub에서 프로젝트 파일을 로컬 컴퓨터로 다운로드합니다. 터미널에 아래 명령어를 입력하세요.

```bash
git clone <여러분의_GitHub_레포지토리_주소>
cd <레포지토리_이름>/Snakegame
```
*(주의: 소스코드와 Makefile이 `Snakegame` 폴더 안에 있으므로, 반드시 해당 폴더로 이동한 후 컴파일해야 합니다.)*

### 3. 컴파일 (Compile)
`Snakegame` 폴더 안에는 컴파일 스크립트인 `Makefile`이 포함되어 있습니다. 아래 명령어를 통해 프로젝트를 빌드합니다.

```bash
make
```
*(`make clean`을 통해 기존 빌드 파일을 삭제한 뒤, 다시 `make`로 재빌드할 수 있습니다.)*

### 4. 게임 실행 (Run)
컴파일이 완료되면 `snakegame` 실행 파일이 생성됩니다. 아래 명령어로 게임을 실행합니다.
...
**⚠️ 실행 전 필수 주의사항:**
- 이 게임은 `ncurses` 기반으로 화면에 직접 출력되므로 **터미널 창 크기**가 매우 중요합니다.
- 실행 전 터미널 창(VS Code 하단 터미널 등)을 위로 드래그하거나 창 크기를 충분히 키운 뒤에 실행해주세요. 창 크기가 너무 작으면 프로그램이 즉시 종료되거나 화면이 깨질 수 있습니다.
